...
11. Decision surface discipline: for full-scale screens, use `references/decision-surface-discipline.md` to separate always-visible decision surface, selected supporting detail, collapsed or external diagnostics, and internal implementation details.
...
12. If a flow frame represents a dense desktop or mobile screen, read `references/decision-surface-discipline.md`, then create a full-scale `1440x1024` desktop or `390x844` mobile wireframe before high-fidelity styling. Before accepting it, run the plain-language review and hide internal IDs or implementation terms that do not change the user's next action.
